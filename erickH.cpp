...
void MaverickMoveAlien(Game *g) 
{
    ...
    alien.pos[1] += alien.vel[1] - rnd() * 5;
    if ((alien.pos[0] < 5) ||
	    (alien.pos[0] > (float)xres+10.0)) {
	if(alien.pos[0] < 5) {
	    alien.pos[0] = 6;
	}
	else if(alien.pos[0] > (float)xres+14.0) {
	    alien.pos[0] = (float)xres+13.0;
	}
	alien.vel[0] = -alien.vel[0];
	//addgrav = 0;
    }
    if ((alien.pos[1] < 5) || 
	    (alien.pos[1] >= (float)yres)) {
	if(alien.pos[1] < 5) {
	    alien.pos[1] =6;
	}
	else if(alien.pos[1] > (float)yres) {
	    alien.pos[1] = (float)yres - 130.0;
	}
